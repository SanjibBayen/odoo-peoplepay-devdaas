    ...
    include: [
      {
        ...
        include: [], // FIX: Prevent nested includes
      },
      {
        model: Department,
        as: 'department',
...
export const createContract = asyncHandler(async (req, res, next) => {
  ...
  if (!employeeId || !contractNumber || !startDate || !wage) {
    throw new AppError('Please provide employeeId, contractNumber, startDate, and wage', 400);
  }
  ...
  if (status === 'ACTIVE' || status === 'DRAFT') {
    const overlappingContract = await Contract.findOne({
      where: {
        ...
        [Op.or]: [
          { startDate: { [Op.between]: [startDate, endDate || '9999-12-31'] } },
          { endDate: { [Op.between]: [startDate, endDate || '9999-12-31'] } },
          {
            [Op.and]: [
              { startDate: { [Op.lte]: startDate } },
              {
                [Op.or]: [
                  { endDate: null },
                  { endDate: { [Op.gte]: endDate || '9999-12-31' } },
                ],
              },
            ],
          },
...
export const updateContract = asyncHandler(async (req, res, next) => {
  ...
  if (contractNumber && contractNumber !== contract.contractNumber) {
    const numberExists = await Contract.findOne({
      where: { contractNumber, id: { [Op.ne]: id } },
    });
    if (numberExists) {
      throw new AppError('Contract number already exists', 400);
    }
  ...
  if (status === 'ACTIVE' || status === 'DRAFT') {
    const overlappingContract = await Contract.findOne({
      where: {
        ...
        status: { [Op.in]: ['ACTIVE', 'DRAFT'] },
        [Op.or]: [
          { startDate: { [Op.between]: [newStartDate, newEndDate || '9999-12-31'] } },
          { endDate: { [Op.between]: [newStartDate, newEndDate || '9999-12-31'] } },
          {
            [Op.and]: [
              { startDate: { [Op.lte]: newStartDate } },
              {
                [Op.or]: [
                  { endDate: null },
                  { endDate: { [Op.gte]: newEndDate || '9999-12-31' } },
                ],
              },
            ],
          },
...
export const activateContract = asyncHandler(async (req, res, next) => {
  ...
  const overlappingContract = await Contract.findOne({
    where: {
      ...
      status: 'ACTIVE',
      [Op.or]: [
        { startDate: { [Op.between]: [contract.startDate, contract.endDate || '9999-12-31'] } },
        { endDate: { [Op.between]: [contract.startDate, contract.endDate || '9999-12-31'] } },
        {
          [Op.and]: [
            { startDate: { [Op.lte]: contract.startDate } },
            {
              [Op.or]: [
                { endDate: null },
                { endDate: { [Op.gte]: contract.endDate || '9999-12-31' } },
              ],
            },
          ],
        },
