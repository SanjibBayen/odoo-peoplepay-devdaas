...
export const createAllocation = asyncHandler(async (req, res, next) => {
  const { employeeId, timeOffTypeId, allocatedAmount, validFrom, validTo, notes } = req.body;

  if (!employeeId || !timeOffTypeId || !allocatedAmount || !validFrom || !validTo) {
    throw new AppError(
      'Please provide employeeId, timeOffTypeId, allocatedAmount, validFrom, and validTo',
      400
    );
  }
