...
export const checkIn = asyncHandler(async(req, res, next) => {
    ...
    let lateMinutes = 0;
    if (scheduledStart) {
        const startTimeStr = typeof scheduledStart === 'string' ? scheduledStart : scheduledStart.toString();
    ...
    res.status(201).json({
        success: true,
        message: lateMinutes > 0 ? `Checked in. Late by ${lateMinutes} minutes` : 'Checked in successfully',
        data: attendance,
    });
});
