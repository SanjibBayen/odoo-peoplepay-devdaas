...
export default class Payslip extends Model {
  ...
  static associate(models) {
    ...
    this.belongsTo(models.SalaryStructure, {
  ...
  async compute() {
    ...
    const deductionRules = rules.filter((r) =>
      ['DEDUCTION', 'TAX', 'CONTRIBUTION'].includes(r.category)
    );
    ...
    this.totalAllowances = allowanceRules.reduce(
      (sum, r) => sum + (processedValues[r.code] || 0),
      0
    );
    this.totalDeductions = deductionRules.reduce(
      (sum, r) => sum + (processedValues[r.code] || 0),
      0
    );
  ...
  async generatePayslipNumber() {
    ...
    const count =
      (await this.sequelize.models.Payslip.count({
